...
public class MergeSort {
	...
	private static <T> void mergeSort(T[] data, Comparator<T> comparator, int leftIndex, int rightIndex) {
	...
	private static <T> void merge(T[] a, Comparator<T> comparator, int leftIndex, int middleIndex, int rightIndex) {
	...
	private static <T> boolean leftGreaterThanRight(Comparator<T> c, T[] leftData, int leftIndex, T[] rightData, int rightIndex) {
		return rightIndex == rightData.length || (leftIndex < leftData.length && c.compare(rightData[rightIndex], leftData[leftIndex]) == 1);
	}
}
